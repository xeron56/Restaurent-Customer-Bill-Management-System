...
{
    public partial class Customer_Entry_Record : Form
    {
        ConnectionString cs = new ConnectionString();
        CommonClasses cc = new CommonClasses();
        public Customer_Entry_Record()
        {
            InitializeComponent();
        }
        public void GetData()
        {
            try
            {
                ...
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(C_ID) as [ID],RTRIM(CustomerID) as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(Address) as [Address],RTRIM(City) as [City],RTRIM(ContactNo) as [Contact No.],RTRIM(Email) as [Email],Photo from Customer order by name", cc.con);
        ...
        private void txtGuestName_TextChanged(object sender, EventArgs e)
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(C_ID) as [ID],RTRIM(CustomerID) as [Customer ID],RTRIM(Name) as [Customer Name],RTRIM(Address) as [Address],RTRIM(City) as [City],RTRIM(ContactNo) as [Contact No.],RTRIM(Email) as [Email],Photo from Customer WHERE name like '" + txtCustomerName.Text + "%' order by name", cc.con);
                cc.da = new SqlDataAdapter(cc.cmd);
                cc.ds = new DataSet();
                cc.da.Fill(cc.ds, "Customer");
                dgw.DataSource = cc.ds.Tables["Customer"].DefaultView;
        ...
        private void dgw_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                if (lblOperation.Text == "Customer Master")
                {
                    ...
                    Customer_Entry frm = new Customer_Entry();
                    frm.Show();
